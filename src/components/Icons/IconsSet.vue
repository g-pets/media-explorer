<template>
	<svg width="0" height="0" viewBox="0 0 0 0" version="1.1" xmlns="http://www.w3.org/2000/svg" class="svg_defs">
		<defs>

			<!-- logo -->
			<symbol id="icon_logo" viewBox="0 0 24 24">
				<g><rect fill="none" height="24" width="24"/><path d="M14,12l-2,2l-2-2l2-2L14,12z M12,6l2.12,2.12l2.5-2.5L12,1L7.38,5.62l2.5,2.5L12,6z M6,12l2.12-2.12l-2.5-2.5L1,12 l4.62,4.62l2.5-2.5L6,12z M18,12l-2.12,2.12l2.5,2.5L23,12l-4.62-4.62l-2.5,2.5L18,12z M12,18l-2.12-2.12l-2.5,2.5L12,23l4.62-4.62 l-2.5-2.5L12,18z"/></g>
			</symbol>

			<!-- rejected -->
			<symbol id="icon_reject" viewBox="0 0 24 24">
				<path d="M3,5v14c0,1.1,0.9,2,2,2h14c1.1,0,2-0.9,2-2V5c0-1.1-0.9-2-2-2H5C3.9,3,3,3.9,3,5z M16.3,16.29L16.3,16.29 c-0.39,0.39-1.02,0.39-1.41,0L12,13.41l-2.89,2.89c-0.39,0.39-1.02,0.39-1.41,0l0,0c-0.39-0.39-0.39-1.02,0-1.41L10.59,12L7.7,9.11 c-0.39-0.39-0.39-1.02,0-1.41l0,0c0.39-0.39,1.02-0.39,1.41,0L12,10.59l2.89-2.88c0.39-0.39,1.02-0.39,1.41,0l0,0 c0.39,0.39,0.39,1.02,0,1.41L13.41,12l2.89,2.88C16.68,15.27,16.68,15.91,16.3,16.29z"/>
			</symbol>

			<!-- show-rejected -->
			<symbol id="icon_show-rejected" viewBox="0 0 24 24">
				<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11H7v-2h10v2z"/>
			</symbol>

			<!-- show-processed -->
			<symbol id="icon_check" viewBox="0 0 24 24">
				<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
			</symbol>

			<!-- author -->
			<symbol id="icon_author" viewBox="0 0 24 24">
				<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/>
			</symbol>

			<!-- fullscreen -->
			<symbol id="icon_fullscreen" viewBox="0 0 24 24">
				<path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/>
			</symbol>
			
			<!-- fullscreen_exit -->
			<symbol id="icon_fullscreen-exit" viewBox="0 0 24 24">
				<path d="M0 0h24v24H0V0z" fill="none"/><path d="M6 16h2v2c0 .55.45 1 1 1s1-.45 1-1v-3c0-.55-.45-1-1-1H6c-.55 0-1 .45-1 1s.45 1 1 1zm2-8H6c-.55 0-1 .45-1 1s.45 1 1 1h3c.55 0 1-.45 1-1V6c0-.55-.45-1-1-1s-1 .45-1 1v2zm7 11c.55 0 1-.45 1-1v-2h2c.55 0 1-.45 1-1s-.45-1-1-1h-3c-.55 0-1 .45-1 1v3c0 .55.45 1 1 1zm1-11V6c0-.55-.45-1-1-1s-1 .45-1 1v3c0 .55.45 1 1 1h3c.55 0 1-.45 1-1s-.45-1-1-1h-2z"/>
			</symbol>

			<!-- settings -->
			<symbol id="icon_settings" viewBox="0 0 24 24">
				<g><path d="M0,0h24v24H0V0z" fill="none"/><path d="M19.14,12.94c0.04-0.3,0.06-0.61,0.06-0.94c0-0.32-0.02-0.64-0.07-0.94l2.03-1.58c0.18-0.14,0.23-0.41,0.12-0.61 l-1.92-3.32c-0.12-0.22-0.37-0.29-0.59-0.22l-2.39,0.96c-0.5-0.38-1.03-0.7-1.62-0.94L14.4,2.81c-0.04-0.24-0.24-0.41-0.48-0.41 h-3.84c-0.24,0-0.43,0.17-0.47,0.41L9.25,5.35C8.66,5.59,8.12,5.92,7.63,6.29L5.24,5.33c-0.22-0.08-0.47,0-0.59,0.22L2.74,8.87 C2.62,9.08,2.66,9.34,2.86,9.48l2.03,1.58C4.84,11.36,4.8,11.69,4.8,12s0.02,0.64,0.07,0.94l-2.03,1.58 c-0.18,0.14-0.23,0.41-0.12,0.61l1.92,3.32c0.12,0.22,0.37,0.29,0.59,0.22l2.39-0.96c0.5,0.38,1.03,0.7,1.62,0.94l0.36,2.54 c0.05,0.24,0.24,0.41,0.48,0.41h3.84c0.24,0,0.44-0.17,0.47-0.41l0.36-2.54c0.59-0.24,1.13-0.56,1.62-0.94l2.39,0.96 c0.22,0.08,0.47,0,0.59-0.22l1.92-3.32c0.12-0.22,0.07-0.47-0.12-0.61L19.14,12.94z M12,15.6c-1.98,0-3.6-1.62-3.6-3.6 s1.62-3.6,3.6-3.6s3.6,1.62,3.6,3.6S13.98,15.6,12,15.6z"/></g>
			</symbol>

			<!-- help -->
			<symbol id="icon_help" viewBox="0 0 24 24">
				<path d="M11 18h2v-2h-2v2zm1-16C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm0-14c-2.21 0-4 1.79-4 4h2c0-1.1.9-2 2-2s2 .9 2 2c0 2-3 1.75-3 5h2c0-2.25 3-2.5 3-5 0-2.21-1.79-4-4-4z"/>
			</symbol>

			<!-- search -->
			<symbol id="icon_search" viewBox="0 0 24 24">
				<path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
			</symbol>

			<!-- filter -->
			<symbol id="icon_filter" viewBox="0 0 24 24">
				<path d="M4.25,5.61C6.27,8.2,10,13,10,13v6c0,0.55,0.45,1,1,1h2c0.55,0,1-0.45,1-1v-6c0,0,3.72-4.8,5.74-7.39 C20.25,4.95,19.78,4,18.95,4H5.04C4.21,4,3.74,4.95,4.25,5.61z"/>
			</symbol>

			<!-- Close Toolbar Sections -->
			<symbol id="icon_sections-close" viewBox="0 0 24 24">
				<path d="M2 6a1 1 0 011-1h18a1 1 0 110 2H3a1 1 0 01-1-1zM2 10a1 1 0 011-1h18a1 1 0 110 2H3a1 1 0 01-1-1zM2 14a1 1 0 011-1h18a1 1 0 110 2H3a1 1 0 01-1-1zM2 18a1 1 0 011-1h18a1 1 0 110 2H3a1 1 0 01-1-1z"/>
			</symbol>

			<!-- Open Toolbar Sections -->
			<symbol id="icon_sections-open" viewBox="0 0 24 24">
				<path d="M2 7c0-1.1.9-2 2-2h16a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2V7zM2 15c0-1.1.9-2 2-2h16a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2v-2z"/>
			</symbol>

			<!-- Solo Toolbar Sections -->
			<symbol id="icon_sections-solo" viewBox="0 0 24 24">
				<path d="M2 6a1 1 0 011-1h18a1 1 0 110 2H3a1 1 0 01-1-1zM2 11c0-1.1.9-2 2-2h16a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2v-2zM2 18a1 1 0 011-1h18a1 1 0 110 2H3a1 1 0 01-1-1z"/>
			</symbol>


			<!-- Small Cell -->
			<symbol id="icon_small-cell" viewBox="0 0 24 24">
				<path d="M3 9h4V5H3v4zm0 5h4v-4H3v4zm5 0h4v-4H8v4zm5 0h4v-4h-4v4zM8 9h4V5H8v4zm5-4v4h4V5h-4zm5 9h4v-4h-4v4zM3 19h4v-4H3v4zm5 0h4v-4H8v4zm5 0h4v-4h-4v4zm5 0h4v-4h-4v4zm0-14v4h4V5h-4z"/>
			</symbol>
			
			<!-- Large Cell -->
			<symbol id="icon_large-cell" viewBox="0 0 24 24">
				<path d="M14.67,5v6.5H9.33V5H14.67z M15.67,11.5H21V5h-5.33V11.5z M14.67,19v-6.5H9.33V19H14.67z M15.67,12.5V19H21v-6.5H15.67z M8.33,12.5H3V19h5.33V12.5z M8.33,11.5V5H3v6.5H8.33z"/>
			</symbol>

			<!-- Star -->
			<symbol id="icon_star" viewBox="0 0 24 24">
				<path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
			</symbol>


			<symbol id="icon_table-view" viewBox="0 0 24 24">
				<path d="M1 4h22v4H1V4zM1 10h22v4H1v-4zM1 16h22v4H1v-4z"/>
			</symbol>

			<symbol id="icon_grid-view" viewBox="0 0 24 24">
				<path d="M1 16h6v4H1v-4zM1 10h6v4H1v-4zM1 4h6v4H1V4zM9 16h6v4H9v-4zM9 10h6v4H9v-4zM9 4h6v4H9V4zM17 16h6v4h-6v-4zM17 10h6v4h-6v-4zM17 4h6v4h-6V4z"/>
			</symbol>
			
			<symbol id="icon_strip-view" viewBox="0 0 24 24">
				<path d="M1 4h22v10H1V4zM1 16h6v4H1v-4zM9 16h6v4H9v-4zM17 16h6v4h-6v-4z"/>
			</symbol>

			<!-- table view (compact) -->
			<symbol id="icon_table-view-compact" viewBox="0 0 24 24">
				<path d="M1 4h22v2.67H1V4zM1 8.44h22v2.67H1V8.44zM1 12.89h22v2.67H1v-2.67zM1 17.33h22V20H1v-2.67z"/>
			</symbol>

			<!-- table view (wide) -->
			<symbol id="icon_table-view-wide" viewBox="0 0 24 24">
				<path d="M1 4h22v7.11H1V4zM1 12.89h22V20H1v-7.11z"/>
			</symbol>

			<!-- grid view (compact) -->
			<symbol id="icon_grid-view-compact" viewBox="0 0 24 24">
				<path d="M1 4h4v2.67H1V4zM1 8.44h4v2.67H1V8.44zM1 12.89h4v2.67H1v-2.67zM1 17.33h4V20H1v-2.67zM7 4h4v2.67H7V4zM7 8.44h4v2.67H7V8.44zM7 12.89h4v2.67H7v-2.67zM7 17.33h4V20H7v-2.67zM13 4h4v2.67h-4V4zM13 8.44h4v2.67h-4V8.44zM13 12.89h4v2.67h-4v-2.67zM13 17.33h4V20h-4v-2.67zM19 4h4v2.67h-4V4zM19 8.44h4v2.67h-4V8.44zM19 12.89h4v2.67h-4v-2.67zM19 17.33h4V20h-4v-2.67z"/>
			</symbol>

			<!-- grid view (wide) -->
			<symbol id="icon_grid-view-wide" viewBox="0 0 24 24">
				<path d="M1 4h10v7H1zM1 13h10v7H1zM13 4h10v7H13zM13 13h10v7H13z"/>
			</symbol>

			<!-- strip view (compact) -->
			<symbol id="icon_strip-view-compact" viewBox="0 0 24 24">
				<path d="M1 4h22v11H1V4zM1 17h4v3H1v-3zM7 17h4v3H7v-3zM13 17h4v3h-4v-3zM19 17h4v3h-4v-3z"/>
			</symbol>

			<!-- strip view (wide) -->
			<symbol id="icon_strip-view-wide" viewBox="0 0 24 24">
				<path d="M1 4h22v9H1V4zM1 15h5v5H1v-5zM18 15h5v5h-5v-5zM8 15h8v5H8v-5z"/>
			</symbol>





			<symbol id="icon_github" viewBox="0 0 24 24">
				<path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/>
			</symbol>
		</defs>
	</svg>
</template>